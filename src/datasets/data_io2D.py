...
def get_texture_iter(folder, npx=128, batch_size=64, \
    ...
    while True:
        data = np.zeros((batch_size, n_channel, npx,
                         npx))  # NOTE: assumes n_channel channels!
...
def get_data_generator(folder, batch_size, npx, filter=False, mirror=True, n_channel=1):
    _gen = get_texture_iter(folder + "/", npx, batch_size, filter, mirror, n_channel)
    while True:
        yield np.swapaxes(next(_gen), 1, -1)
