...
# Slightly modified version of the 2D SGAN code by Jetchev et al. (2016)
...
conv = lambda incoming, num_filters, filter_size, W, b, nonlinearity: \
    lasagne.layers.Conv2DLayer(incoming, num_filters, filter_size, stride=(2, 2), pad='same', W=W, b=b,
                               flip_filters=True, nonlinearity=nonlinearity)
tconv = lambda incoming, num_filters, filter_size, W, nonlinearity: lasagne.layers.TransposedConv2DLayer(incoming,
                                                                                                         num_filters,
                                                                                                         filter_size,
                                                                                                         stride=(2, 2),
                                                                                                         crop='same',
...
class SGAN(object):
    def __init__(self, name=None):
        ...
        if name is not None:
            ...
            ##now overwrite the static config with the correct values - comment by ELa: doesn't work and not necessary
            #            self.config.gen_ks = []
            #            self.config.gen_fn = []
            #            l = len(vals["gen_W"])
            #            for i in range(l):
    ...
    def _sample_initials(self):
        ...
        self.dis_W.append(
            sharedX(
                w_init.sample((self.dis_fn[0], self.config.nc, self.dis_ks[0][
                    0], self.dis_ks[0][1]))))
        for l in range(self.dis_depth - 1):
            self.dis_W.append(
                sharedX(
                    w_init.sample((self.dis_fn[l + 1], self.dis_fn[
                        l], self.dis_ks[l + 1][0], self.dis_ks[l + 1][1]))))
        ...
        for l in range(self.gen_depth - 1):
            self.gen_W += [
                sharedX(
                    w_init.sample((last, self.gen_fn[l], self.gen_ks[l][0],
                                   self.gen_ks[l][1])))
            ]
            last = self.gen_fn[l]
        ...
        self.gen_W += [
            sharedX(
                w_init.sample((last, self.gen_fn[-1], self.gen_ks[-1][0],
                               self.gen_ks[-1][1])))
        ]
    ...
    def _spatial_discriminator(self, inlayer):
        ...
        # layers  = [inlayer]
        layers = [
            gauss_noise(inlayer, sigma=0.1)
        ]  # Corrupt the inlayer provided to the discriminator with white noise to stabilize training (see Sonderby et al. [2016])
        layers.append(
            conv(
                layers[-1],
                self.dis_fn[0],
    ...
    def _build_sgan(self):
        ##
        # network
        Z = lasagne.layers.InputLayer(
            (None, self.config.nz, None,
             None))  # leave batch_size and shape unspecified for now
        X = lasagne.layers.InputLayer((
            self.config.batch_size, self.config.nc, self.config.npx,
            self.config.npx))  # leave batch_size and shape unspecified for now
        ...
        obj_d = -T.mean(T.log(1 - prediction_fake)) - T.mean(
            T.log(prediction_real)) + self.config.l2_fac * l2_dis
...
if __name__ == "__main__":
    ...
    z_sample = np.random.uniform(
        -1., 1., (1, c.nz, c.zx_sample,
                  c.zx_sample))  # the sample for which to plot samples
    ...
    while (epoch <= num_epoch):
        ...
        save_tensor(data[0], 'samples/%s_epoch%d.png' % (c.save_name, epoch))
