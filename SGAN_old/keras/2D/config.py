...
class Config(object):
    ...
    gen_fn = gen_fn[::-1]
    dis_fn = [2**(n + 6) for n in range(dis_ls - 1)
              ] + [1]  # discriminative number of filters
    ...
    save_name = sub_name + "_filters%d_npx%d_%dgL_%ddL" % (dis_fn[0], npx,
                                                           gen_ls, dis_ls)
    ...
    def print_info(cls):
        ...
        print("Learning and generating samples from zx ", cls.zx,
              ", which yields images of size npx ",
              zx_to_npx(cls.zx, cls.gen_ls))
        print("Producing samples from zx_sample ", cls.zx_sample,
              ", which yields) images of size npx ",
              zx_to_npx(cls.zx_sample, cls.gen_ls))
